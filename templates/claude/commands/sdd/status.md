...
Call `mcp__team-sdd__sdd_get_context` first, then call `mcp__team-sdd__sdd_status` with the active project root and Delivery ID. Treat both tool responses as internal structured data. Do not show raw MCP JSON, envelopes, or a separate Core-result summary.
