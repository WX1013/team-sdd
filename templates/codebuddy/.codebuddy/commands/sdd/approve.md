...
为 `$1` 调用 `mcp__team-sdd__sdd_get_context`，向用户确认 `$2` 和 `$3` 后调用 `mcp__team-sdd__sdd_approve`。将工具响应仅作为内部结构化数据处理。面向用户的内容必须使用简体中文；不得展示原始 JSON、MCP 响应包络或 Core 结果原文。保留 Delivery/Spec ID、状态枚举和错误码。
